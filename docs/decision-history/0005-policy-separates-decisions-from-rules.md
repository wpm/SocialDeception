...
The name *policy* is borrowed from reinforcement learning on purpose: the
thing that maps a state to a distribution over actions.
...
The reinforcement-learning triple is named through, so the code and the
theory share one language and no synonyms are introduced.
...
The set of permitted actions is the **action space**, computed by
`Player::action_space` and `View::action_space`. An action not in it is
...
```mermaid
flowchart LR
    E["Event&lt;Message&gt;<br/>observation"] -->|fold| K["Knowledge<br/>state"]
    K -->|rules| S["Vec&lt;Action&gt;<br/>action space"]
    K --> P{{"Policy::choose"}}
    S --> P
    P -->|one Action| R["Response"]
```
...
Narration is addressed, not broadcast: to one player, to the living, or to
the pack. That choice of recipients is the whole of the hidden-information
mechanism, and only the final outcome goes to everyone. A player observes
many narrations but acts only on a request, which is what gives an agent in a
turnless runtime its decision points.

Dialogue, when it arrives, is a fourth kind: player-to-player speech. Unlike
narration it may be false, so the boundary between knowledge and belief will
become visible in the message type itself.

### `Knowledge` is not the observation space

An observation is one event. `Knowledge` is the state a policy conditions
on, folded from every observation so far. Calling it the observation space
would confuse the thing observed with the summary of everything observed.

### `Knowledge` is not a belief

"Belief" is reserved for information that could be false, and nothing in
`Knowledge` can be: every observation a player receives is a true statement
from the moderator. The state is *incomplete* about the hidden world and
never *incorrect* about it. A villager does not know who the werewolves are,
but nothing it does know is wrong. The name is meant in its ordinary sense,
in which knowing something entails its being so.

That boundary will move. When dialogue arrives, players will assert things
that may be lies — that is what a social deception game is — and whatever
holds those assertions, and the conclusions drawn from them, will be beliefs
and will want a type of its own. The line is already drawn: `Knowledge`
folds only moderator narration, and inference is the policy's business
rather than the state's. A villager hearing a day tally stores the tally,
which is true, and draws whatever conclusions it likes in its policy, where
being wrong costs it nothing structural.
...
all. A function with no caller is the same mistake as `sensible`, below.

The observation space is a real notion even so, because it is per role. A
villager can never observe a night `Tally`, an `Investigated`, or a non-empty
pack; those lie outside a villager's observation space, and that they never
occur in one is exactly what the hidden-information invariants assert. "The
realized observations stay inside each role's observation space" is the
precise statement of what routing enforces.

### Where the line between rules and strategy falls
...
A policy may block. ADR-0001 grants every agent its own thread precisely so
that a handler waiting on a model provider blocks only itself, and most of
"leaving a path open to a language model" was decided there.
...
1. It would have exactly one consumer forever. A language-model policy makes
   its own strategic judgments, so only the baseline would ever read it. A
   mechanism with one caller, built into the type system as though it were
   general, is the premature generalization this project restarted to avoid.
2. Handing it to a model would contaminate the measurement. The win rate
   would be partly the model's play and partly the list's, with no clean
   ablation between them.
3. It bought nothing. The stated justification was that strategic filters
   need private knowledge, but that knowledge is in `Knowledge`, which is in
   the `View`, so any policy can compute those filters itself.

### Strategic filtering inside the role

The role could simply omit strategically poor actions from the action space.
Rejected because it makes the baseline's prior part of the definition of the
game, where it can never be weakened or ablated, and because trajectories
produced under it could not teach a learner about the omitted moves.

### A fallible `choose`

`Policy::choose` could return an error for the rules to handle. Rejected
because there is no correct handling: the game cannot skip a decision, and
substituting a random action is a policy decision that belongs in the policy.
...
- **The uniform baseline is a real measurement,** establishable without a
  model. With the seer's information unused and werewolves indistinguishable
  from villagers, its win rate is a function of the role counts alone.
  Running many episodes to measure it is out of scope for the Werewolf
  epic; this design is what makes it a small addition later.
...
- **A policy violation is a panic,** not a recoverable error. That is the
  right severity for a bug in the code that was supposed to choose.
- **The determinism claim is testable** as stated: two runs of one
  configuration and seed must produce identical logical transcripts, and a
  test that compares them must first project out timestamps and record
  interleaving.
